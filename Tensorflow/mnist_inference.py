...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        ...
        '''input_tensor:输入的图片像素矩阵, input_tensor[0,:,:,:]表示输入的第一张图片...
           conv1_weights:创建过滤器的权重,
           strides:不同维度上的步长,其中第一维和最后一维必须是 1，因为卷积层的步长只对矩阵的长和宽有效.
        '''
    ...
    with tf.name_scope('layer2-pool'):
        ...
        '''relu1:传入的当前节点矩阵
           ksize:过滤器的尺寸
           strides:步长,[1,2,2,1]第一维和最后一维必须是1
           padding:全0填充
        '''
